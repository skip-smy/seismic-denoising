...
def parse_args():
    ...
    parser.add_argument('--noisy_npy_path', default='data/real_noisy.npy', type=str, help='path of noisy npy (unnormalized)')
    parser.add_argument('--clean_npy_path', default='data/clean.npy', type=str, help='path of clean npy (unnormalized)')
...
def main():
    ...
    # 3. 读取 noisy.npy 和 clean.npy（未归一化）
    if not os.path.exists(args.noisy_npy_path):
        raise FileNotFoundError(f'noisy.npy 不存在: {args.noisy_npy_path}')
    if not os.path.exists(args.clean_npy_path):
        raise FileNotFoundError(f'clean.npy 不存在: {args.clean_npy_path}')

    x_noisy_raw = np.load(args.noisy_npy_path).astype(np.float32)
    x_clean_raw = np.load(args.clean_npy_path).astype(np.float32)

    if x_noisy_raw.shape != x_clean_raw.shape:
        raise ValueError(f'noisy 和 clean 尺寸不一致: {x_noisy_raw.shape} vs {x_clean_raw.shape}')

    # 4. 用同一组参数归一化到 [0, 1]
    pmin, pmax = np.percentile(x_clean_raw, 1), np.percentile(x_clean_raw, 99)
    x_clean = 2* (x_clean_raw - pmin) / (pmax - pmin + 1e-12) - 1
    x_noisy = 2* (x_noisy_raw - pmin) / (pmax - pmin + 1e-12) - 1

    x_clean = np.clip(x_clean, -1, 1).astype(np.float32)
    x_noisy = np.clip(x_noisy, -1, 1).astype(np.float32)
    ...
    data_name = os.path.splitext(os.path.basename(args.noisy_npy_path))[0]
    ...
    for set_cur in args.set_names:
        ...
        if args.save_result:
            ...
            np.save(os.path.join(set_result_dir, data_name + '_clean_norm.npy'), x_clean)
            np.save(os.path.join(set_result_dir, data_name + '_noisy_norm.npy'), x_noisy)
            np.save(os.path.join(set_result_dir, data_name + '_dncnn_denoised_norm.npy'), x_denoised)
        
            with open(os.path.join(set_result_dir, 'results.txt'), 'w', encoding='utf-8') as f:
                f.write("="*20)
                f.write(f"{time.strftime('%Y-%m-%d %H:%M:%S')}\n")
                f.write(f"0-1区间归一化")
                f.write(f"noisy文件: {args.noisy_npy_path}\n")
                f.write(f"clean文件: {args.clean_npy_path}\n")
