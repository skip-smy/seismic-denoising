...
class Model:
    def __init__(self, opt: Dict[str, Any]):
        ...
        self.save_dir: str = opt['path'].get('models', opt['path']['images'])
    ...
    def load_network(self, load_path: str, network: Union[nn.DataParallel,
        ...
        strict_load = self.opt['train'].get('load_strict', True)

        network.head.load_state_dict(
            torch.load(os.path.join(load_path, 'head.pth'),
                       map_location=self.device),
            strict=strict_load)

        state_dict_x = torch.load(os.path.join(load_path, 'x.pth'),
                                  map_location=self.device)
        network.body.net_x.load_state_dict(state_dict_x, strict=strict_load)

        state_dict_d = torch.load(os.path.join(load_path, 'd.pth'),
                                  map_location=self.device)
        network.body.net_d.load_state_dict(state_dict_d, strict=strict_load)

        state_dict_hypa = torch.load(os.path.join(load_path, 'hypa.pth'),
                                     map_location=self.device)
        if self.opt['train']['reload_broadcast']:
            ...
            for hypa in network.hypa_list:
                hypa.load_state_dict(state_dict_hypa_v2, strict=strict_load)
        else:
            network.hypa_list.load_state_dict(state_dict_hypa,
                                              strict=strict_load)
