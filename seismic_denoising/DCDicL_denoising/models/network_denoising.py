...
class SolveFFT(nn.Module):
    ...
    def forward(self, X: Tensor, D: Tensor, Y: Tensor, alpha: Tensor,
        ...
        return irfft(X, 2, signal_sizes=list(x_size))
...
class SolveLS(nn.Module):
    ...
    def forward(self, x, d, y, alpha, reg):
        ...
        # reg
        alpha = alpha.to(dtype=xtx.dtype)
        alpha = alpha * x.size(3) * x.size(4) * reg / (d_size**2 * d.size(2))
        alpha_diag = alpha.squeeze(-1).squeeze(-1).to(dtype=xtx.dtype)
        diag_idx = torch.arange(xtx.size(1), device=xtx.device)
        xtx[:, diag_idx, diag_idx] = xtx[:, diag_idx, diag_idx] + alpha_diag
        xty += alpha.squeeze(-1).to(dtype=xty.dtype) * d.reshape(
            d.size(0), d.size(1), -1).permute(0, 2, 1)
