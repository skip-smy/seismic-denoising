...
def rfft(x: torch.Tensor, signal_ndim: int):
    dims = tuple(range(-signal_ndim, 0))
    return torch.view_as_real(torch.fft.fftn(x, dim=dims))


def irfft(x: torch.Tensor, signal_ndim: int, signal_sizes):
    dims = tuple(range(-signal_ndim, 0))
    x_complex = torch.view_as_complex(x.contiguous())
    return torch.fft.ifftn(x_complex, s=signal_sizes, dim=dims).real
